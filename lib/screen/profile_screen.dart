...
class _ProfileScreenState extends State<ProfileScreen> {
  int selectedView = 0;
  String username = "username"; // Valeur fictive pour remplacer Firebase
  String avatarUrl =
      "https://example.com/avatar.jpg"; // Valeur fictive pour remplacer Firebase
  ...
  void changeAccount(String name, String url) {
    setState(() {
      showModalBottomSheet(
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        leading: IconButton(
          onPressed: () => addFriend(),
          icon: Icon(Icons.person_add_alt_1_rounded, color: backgroundColor),
        ),
        actions: [
          IconButton(
            onPressed: () => settings(),
            icon: Icon(Icons.more_horiz, color: backgroundColor),
          )
        ],
        backgroundColor: tipo,
      ...
      body: SingleChildScrollView(
        child: Column(
          children: [
            ...
            Stack(children: [
              CircleAvatar(
                  radius: 55, backgroundImage: NetworkImage(avatarUrl)),
              Positioned(
                bottom: 0,
                right: 0,
                child: CircleAvatar(
                  ...
                  child: Center(child: Icon(Icons.add, color: tipo)),
                ),
              )
            ]),
            const SizedBox(height: 10),
            ...
            const Row(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                Stats(number: '100', type: 'Following'),
                SizedBox(width: 30),
                Stats(number: '3', type: 'Followers'),
                SizedBox(width: 30),
                Stats(number: '0', type: 'Likes'),
              ],
            ),
            const SizedBox(height: 10),
            Row(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                Button(name: Text('Edit profile', style: tipoStyle3)),
                const SizedBox(width: 10),
                Button(name: Text('Share profile', style: tipoStyle3)),
                const SizedBox(width: 10),
                Button(
                    name: Icon(Icons.person_add_sharp, color: backgroundColor)),
              ],
            ),
            const SizedBox(height: 10),
            Container(
              ...
              child: Text('+ add bio', style: tipoStyle3),
            ),
            const SizedBox(height: 10),
            Row(
              ...
              children: [
                GestureDetector(
                  ...
                  child: ActionBarWidget(
                      icon: Icons.menu_outlined, isSelected: selectedView == 0),
                ),
                GestureDetector(
                  child: ActionBarWidget(
                ...
                GestureDetector(
                  onTap: () => changeViews(2),
                  child: ActionBarWidget(
                      icon: Icons.rotate_right, isSelected: selectedView == 2),
                ),
                GestureDetector(
                  onTap: () => changeViews(3),
                  child: ActionBarWidget(
